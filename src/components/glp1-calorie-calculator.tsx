...
export function Glp1CalorieCalculator() {
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-white/5 p-8 shadow-2xl shadow-black/20">
      <h1 className="text-3xl font-semibold tracking-tight text-white">GLP-1 Calorie Calculator</h1>

      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        <label className="flex flex-col">
          <span className="text-sm text-zinc-400">Weight unit</span>
          <select value={unitW} onChange={(e) => setUnitW(e.target.value)} className="mt-2 rounded-md bg-black/10 px-3 py-2 text-white">
        ...
        <label className="flex flex-col">
          <span className="text-sm text-zinc-400">Weight ({unitW})</span>
        ...
        <label className="flex flex-col">
          <span className="text-sm text-zinc-400">Height unit</span>
          <select value={unitH} onChange={(e) => setUnitH(e.target.value)} className="mt-2 rounded-md bg-black/10 px-3 py-2 text-white">
        ...
        <label className="flex flex-col">
          <span className="text-sm text-zinc-400">Height ({unitH})</span>
        ...
        <label className="flex flex-col">
          ...
          <input type="number" value={age} onChange={(e) => setAge(Number(e.target.value))} className="mt-2 rounded-md bg-black/10 px-3 py-2 text-white" />
        </label>

        <label className="flex flex-col">
          <span className="text-sm text-zinc-400">Sex</span>
          <select value={sex} onChange={(e) => setSex(e.target.value)} className="mt-2 rounded-md bg-black/10 px-3 py-2 text-white">
        ...
        <label className="flex flex-col">
          ...
          <select value={goal} onChange={(e) => setGoal(e.target.value)} className="mt-2 rounded-md bg-black/10 px-3 py-2 text-white">
      ...
      <div className="mt-6 rounded-lg bg-black/10 p-5">
        <p className="text-sm text-zinc-300">Direct answer: your estimated TDEE is <strong>{tdee} kcal/day</strong>. A GLP-1-aware target is about <strong>{targetCalories} kcal/day</strong> (adjusted for activity and a conservative deficit to protect lean mass).</p>

        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <p className="text-xs text-zinc-400">Estimated BMR</p>
            <div className="mt-1 text-2xl font-semibold text-white">{Math.round(bmr)} kcal</div>
          </div>
          <div>
            <p className="text-xs text-zinc-400">Estimated TDEE</p>
            <div className="mt-1 text-2xl font-semibold text-white">{tdee} kcal</div>
          </div>
        </div>
